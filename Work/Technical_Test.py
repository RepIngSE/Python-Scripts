#Prueba técnica de una base de datos 
...
#Punto #1.3: Mostrar los resultados de enero de 2023 en un gráfico de línea de tiempo
...
#Gráfico de dispersión para el mes de septiembre de 2022
df ["Date"] = pd.to_datetime(df["Date"])
...
#Gráfico comparativo entre volumen de llamadas y nivel de servicio
df ["Date"] = pd.to_datetime(df["Date"])
Septiembre_2022 = df[(df["Date"].dt.year == 2022) & (df["Date"].dt.month == 9)]
...
#Marco de datos ponderados por mes en el año 2021 para el campo IB AHT
df['Date'] = pd.to_datetime(df['Date'])
